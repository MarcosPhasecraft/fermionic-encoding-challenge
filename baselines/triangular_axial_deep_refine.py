...
Total-weight-focused, ancilla-free (N = M), and one uniform rule for every
triangular lattice size.  This self-contained submission combines balanced
ternary Majoranas, graph-aware placement in the three triangular axial
coordinates, and global Clifford refinement.
...
def _hamiltonian(spec):
    """The challenge's full Num + ReHop + ImHop + Inter term list."""
    terms = []
    ...
    for mode in range(spec["M"]):
        terms.append((2 * mode, 2 * mode + 1))
    for left, right in spec["edges"]:
        terms.extend([
            (2 * left, 2 * left + 1),
            (2 * right, 2 * right + 1),
            (2 * left, 2 * left + 1, 2 * right, 2 * right + 1),
        ])
    return terms
...
def _jw_path_topology(m):
    """The Jordan--Wigner path, used only as a refinement initializer.

    Its linear strings complement the logarithmic-depth ternary trees.  It
    enters the exact same placement and Clifford search and is never selected
    by a hard-coded size rule.
    """
    pairs = []
    for slot in range(m):
        prefix = (1 << slot) - 1
        local = 1 << slot
        pairs.append(((local, prefix), (local, prefix | local)))
    parents = [None] + list(range(m - 1))
    return pairs, parents
...
def _axial_order(spec, priority):
    """Recursive ternary placement in x, y, x-y coordinates.

    Each coordinate is constant along one triangular-lattice bond direction.
    Rotating the tie-breaking priority produces distinct, deterministic
    placement basins while remaining the same formula at every size.
    """
    coords = spec["coords"]
    ...
    def recurse(indices, depth):
        if len(indices) <= 1:
            return list(indices)
        values = [projection(index) for index in indices]
        spans = [max(row[k] for row in values) - min(row[k] for row in values)
                 for k in range(3)]
        axes = [(priority + depth + offset) % 3 for offset in range(3)]
        axis = max(axes, key=lambda k: (spans[k], -axes.index(k)))
        place = axes.index(axis)
        secondary = axes[(place + 1) % 3]
        tertiary = axes[(place + 2) % 3]
        indices = sorted(indices,
                         key=lambda i: (projection(i)[axis],
                                        projection(i)[secondary],
                                        projection(i)[tertiary]))
        base, remainder = divmod(len(indices), 3)
        out, start = [], 0
        for child in range(3):
            size = base + (child < remainder)
            group = indices[start:start + size]
            if (depth + child + priority) & 1:
                group = list(reversed(group))
            out.extend(recurse(group, depth + 1))
            start += size
        return out

    return recurse(sorted(coords), 0)


def _spatial_orders(spec):
    """Cartesian control plus the three triangular axial orientations."""
    orders = [_spatial_order(spec)]
    orders.extend(_axial_order(spec, priority) for priority in range(3))
    unique = []
    seen = set()
    for order in orders:
        key = tuple(order)
...
_CARTESIAN_SEEDS = 5  # preserves every placement basin of the control
_FINALISTS = 4        # finalists retained from each topology family
_SURVIVORS = 1        # first deep pass chooses the basin to continue
_PASSES = 30          # deep-refinement passes, stopped early at a fixed point
_ANNEAL_RUNS = 5      # independent escape anneals per pass; keep the best
_PATIENCE = 5         # stochastic null passes tolerated before stopping
#
...
def _deep_refine(finalists, terms, m, pool, passes, runs, steps, tag):
    ...
    for index in range(passes):
        ...
        for state in states:
            ...
            if new_total < total:
                ...
                state[5] = new_total
                improved = True
        if index == 0 and len(states) > _SURVIVORS:
            states.sort(key=lambda state: (state[5], int(state[2].max())))
            states = states[:_SURVIVORS]
            continue
    ...
    best = min(states, key=lambda state: (state[5], int(state[2].max())))
    return best[0], best[5]


def encode(spec):
    m = spec["M"]
    terms = _hamiltonian(spec)
    if m < 2:
    ...
    starts = _spatial_orders(spec)
    cartesian = starts[0]
    axial = starts[1:]
    schedules = sorted({100_000, max(100_000, 2500 * m)})
    heap_starts = [(m + i, cartesian) for i in range(_CARTESIAN_SEEDS)]
    heap_starts.extend((m + _CARTESIAN_SEEDS + i, start)
                       for i, start in enumerate(axial))
    balanced_starts = [(31 * m + 4, cartesian)]
    balanced_starts.extend((31 * m + 5 + i, start)
                           for i, start in enumerate(axial))
    jw_starts = [(43 * m + 11 + i, start)
                 for i, start in enumerate(starts)]
    topologies = [("heap", *_heap_topology(m), heap_starts),
                  ("balanced", *_balanced_topology(m), balanced_starts),
                  ("jw-path", *_jw_path_topology(m), jw_starts)]

    # ---- stage 1+2: placement, then greedy tree-axis Clifford descent ----
    candidates = []
    candidate_groups = []
    for _, tree_pairs, parents, seeded_starts in topologies:
        group = []
        seen = set()
        for steps in schedules:
            for seed, start in seeded_starts:
                ...
                candidate = (total, cap, codes, seed)
    ...
    for group in candidate_groups:
        group.sort(key=lambda c: (c[0], c[1]))
        finalists.extend(group[:_FINALISTS])
    pool = np.array([(i, j) for i in range(m) for j in range(i + 1, m)],
                    dtype=np.int32)
    anneal_steps = max(200_000, _ANNEAL_STEPS_PER_PAIR * len(pool))

    best_codes, _ = _deep_refine(finalists, terms, m, pool,
