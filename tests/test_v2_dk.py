...
_BOTH_EVEN_SIZES = [(4, 4), (6, 6), (4, 6), (8, 8)]
_DEGENERATE_SIZES = [(1, 1), (1, 3), (3, 1)]


@pytest.mark.parametrize("lx,ly", _CASE_I_SIZES)
...
@pytest.mark.parametrize("lx,ly", _BOTH_EVEN_SIZES)
def test_both_even_sizes_are_rejected_with_a_clear_error_not_a_crash(lx, ly):
    spec = build_spec(lx, ly, None)
    with pytest.raises(ValueError, match="both even"):
        encode(spec)


@pytest.mark.parametrize("lx,ly", _BOTH_EVEN_SIZES)
def test_evaluate_extended_reports_both_even_rejection_as_a_structured_failure(lx, ly):
    spec = build_spec(lx, ly, None)
    terms = hamiltonian_terms(spec, model="full")
    result = evaluate_extended(spec, encode, terms, represent_fn=represent)
